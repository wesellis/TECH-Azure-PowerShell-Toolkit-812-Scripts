...
function Initialize-SpringCLI {
    try {
        Write-Host "Checking Azure CLI Spring extension..." "Info"
        $springExtension = az extension list --query "[?name=='spring'].name" -o tsv
        if (-not $springExtension) {
            Write-Host "Installing Azure CLI Spring extension..." "Info"
            az extension add --name spring
            Write-Host "Successfully installed Spring extension" "Success"
        } else {
            Write-Host "Spring extension is already installed" "Success"
        }
        # Update to latest version
        az extension update --name spring
    } catch {
        Write-Host "Failed to initialize Spring CLI: $($_.Exception.Message)" "Error"
        throw
    }
}
...
function New-SpringAppsInstance -ErrorAction Stop {
    ...
    if ($PSCmdlet.ShouldProcess("Spring Apps instance '$SpringAppsName'", "Create")) {
        try {
        Write-Host "Creating Azure Spring Apps instance: $SpringAppsName" "Info"
        # Check if instance already exists
        $existing = az spring show --name $SpringAppsName --resource-group $ResourceGroupName --query "name" -o tsv 2>$null
        if ($existing) {
            Write-Host "Spring Apps instance already exists: $SpringAppsName" "Warning"
            return
        }
        ...
        if ($VNetName -and $SubnetName) {
            ...
            if ($vnet) {
                ...
                if ($subnet) {
                    ...
                    Write-Host "Configuring VNet integration" "Info"
                }
            }
        }
        # Execute creation
        ...
        if ($LASTEXITCODE -eq 0) {
            Write-Host "Successfully created Spring Apps instance: $SpringAppsName" "Success"
        } else {
        ...
        do {
            ...
            Write-Host "Instance provisioning state: $status" "Info"
        } while ($status -eq "Creating")
        if ($status -eq "Succeeded") {
            Write-Host "Spring Apps instance is ready for use" "Success"
        } else {
            throw "Spring Apps instance provisioning failed: $status"
        }
    } catch {
            Write-Host "Failed to create Spring Apps instance: $($_.Exception.Message)" "Error"
            throw
        }
    }
}
...
function Set-SpringCloudService -ErrorAction Stop {
    ...
    if ($PSCmdlet.ShouldProcess("Spring Cloud services for '$SpringAppsName'", "Configure")) {
        try {
        Write-Host "Configuring Spring Cloud services..." "Info"
        # Configure Config Server
        if ($EnableConfigServer) {
            Write-Host "Enabling Spring Cloud Config Server..." "Info"
            if ($ConfigServerRepo) {
...
"@
            ...
            Write-Host "Successfully configured Config Server" "Success"
        }
        # Enable Service Registry (Eureka)
        if ($EnableServiceRegistry) {
            Write-Host "Enabling Service Registry..." "Info"
            # Service Registry is enabled by default in Standard/Enterprise tiers
            Write-Host "Service Registry is available for application registration" "Success"
        }
        # Configure Gateway
        if ($EnableGateway) {
            Write-Host "Enabling Spring Cloud Gateway..." "Info"
            az spring gateway update --name $SpringAppsName --resource-group $ResourceGroupName --assign-endpoint true
            Write-Host "Successfully configured Spring Cloud Gateway" "Success"
        }
    } catch {
            Write-Host "Failed to configure Spring Cloud services: $($_.Exception.Message)" "Error"
        }
    }
}
...
function Install-SpringApplication {
    try {
        Write-Host "Deploying Spring application: $AppName" "Info"
        ...
        if (-not $existingApp) {
            Write-Host "Creating Spring application: $AppName" "Info"
            ...
            Write-Host "Successfully created application: $AppName" "Success"
        }
        # Deploy application
        if ($ArtifactPath -and (Test-Path $ArtifactPath)) {
            Write-Host "Deploying artifact: $ArtifactPath" "Info"
            $deployCmd = @(
            ...
            & $deployCmd | Out-Null
            if ($LASTEXITCODE -eq 0) {
                Write-Host "Successfully deployed application: $AppName" "Success"
            } else {
                throw "Failed to deploy application"
            }
        } else {
            Write-Host "No artifact path specified or file not found, skipping deployment" "Warning"
        }
        ...
        if ($appUrl) {
            Write-Host "Application URL: $appUrl" "Success"
        }
    } catch {
        Write-Host "Failed to deploy Spring application: $($_.Exception.Message)" "Error"
        throw
    }
}
...
function Set-SpringAppScale -ErrorAction Stop {
    ...
    if ($PSCmdlet.ShouldProcess("Spring application '$AppName'", "Scale")) {
        try {
        Write-Host "Scaling Spring application: $AppName" "Info"
        Write-Host "Target instances: $InstanceCount, CPU: $CpuCount, Memory: $($MemoryInGB)Gi" "Info"
        ...
        if ($LASTEXITCODE -eq 0) {
            Write-Host "Successfully scaled application: $AppName" "Success"
        } else {
            throw "Failed to scale application"
        }
    } catch {
            Write-Host "Failed to scale Spring application: $($_.Exception.Message)" "Error"
            throw
        }
    }
}
...
function Set-SpringMonitoring -ErrorAction Stop {
    ...
    if ($PSCmdlet.ShouldProcess("Spring Apps monitoring configuration for '$SpringAppsName'", "Configure")) {
        try {
            Write-Host "Configuring monitoring for Spring Apps..." "Info"
            ...
            if ($EnableApplicationInsights) {
                ...
                if (-not $existingInsights) {
                    Write-Host "Creating Application Insights: $appInsightsName" "Info"
                    ...
                    Write-Host "Successfully created Application Insights" "Success"
                } else {
                ...
                Write-Host "Successfully integrated Application Insights" "Success"
            }
            ...
            Write-Host "Successfully configured  monitoring" "Success"
        } catch {
            Write-Host "Failed to configure monitoring: $($_.Exception.Message)" "Error"
        }
    }
}
...
function Get-SpringAppsStatus -ErrorAction Stop {
    try {
        Write-Host "Monitoring Spring Apps instance status..." "Info"
        # Get instance details
        $instance = az spring show --name $SpringAppsName --resource-group $ResourceGroupName --output json | ConvertFrom-Json
        Write-Host "Spring Apps Instance Status:" "Info"
        Write-Host "Name: $($instance.name)" "Info"
        Write-Host "Location: $($instance.location)" "Info"
        Write-Host "Provisioning State: $($instance.properties.provisioningState)" "Info"
        Write-Host "Service ID: $($instance.properties.serviceId)" "Info"
        Write-Host "Network Profile: $($instance.properties.networkProfile.outboundType)" "Info"
        # Get applications
        $apps = az spring app list --service $SpringAppsName --resource-group $ResourceGroupName --output json | ConvertFrom-Json
        if ($apps) {
            Write-Host "Applications:" "Info"
            foreach ($app in $apps) {
                Write-Host "  - Name: $($app.name)" "Info"
                Write-Host "    State: $($app.properties.provisioningState)" "Info"
                Write-Host "    Public: $($app.properties.public)" "Info"
                Write-Host "    URL: $($app.properties.url)" "Info"
                ...
                foreach ($deployment in $deployments) {
                    Write-Host "    Deployment: $($deployment.name) - Status: $($deployment.properties.status)" "Info"
                    Write-Host "    Instances: $($deployment.properties.deploymentSettings.resourceRequests.cpu) CPU, $($deployment.properties.deploymentSettings.resourceRequests.memory) Memory" "Info"
                }
            }
        } else {
            Write-Host "No applications deployed" "Info"
        }
        Write-Host "Spring Apps monitoring completed" "Success"
    } catch {
        Write-Host "Failed to monitor Spring Apps: $($_.Exception.Message)" "Error"
    }
}
...
function Invoke-AppLifecycleAction {
    ...
    try {
        Write-Host "Executing $LifecycleAction action on application: $AppName" "Info"
        ...
        if ($LASTEXITCODE -eq 0) {
            Write-Host "Successfully executed $LifecycleAction action" "Success"
        } else {
            throw "Failed to execute $LifecycleAction action"
        }
    } catch {
        Write-Host "Failed to execute lifecycle action: $($_.Exception.Message)" "Error"
        throw
    }
}
# Main execution
try {
    Write-Host "Starting Azure Spring Apps Management Tool" "Info"
    Write-Host "Action: $Action" "Info"
    Write-Host "Spring Apps Name: $SpringAppsName" "Info"
    ...
    switch ($Action) {
        ...
        "Delete" {
            Write-Host "Deleting Spring Apps instance: $SpringAppsName" "Warning"
            az spring delete --name $SpringAppsName --resource-group $ResourceGroupName --yes
            if ($LASTEXITCODE -eq 0) {
                Write-Host "Successfully deleted Spring Apps instance" "Success"
            } else {
                throw "Failed to delete Spring Apps instance"
            }
        }
    }
    Write-Host "Azure Spring Apps Management Tool completed successfully" "Success"
} catch {
