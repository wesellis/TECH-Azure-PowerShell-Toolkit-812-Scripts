...
param (
    ...
    [Parameter(Mandatory=$false, HelpMessage="Specific resource group to analyze")]
    [string]$WEResourceGroupName,
    
    [Parameter(Mandatory=$false, HelpMessage="Analysis scope")]
    [ValidateSet("CostAnalysis", "RightsizingRecommendations", "UnusedResources", "SecurityCompliance", "FullOptimization")]
    [string]$WEAnalysisType = "FullOptimization",
    
    [Parameter(Mandatory=$false, HelpMessage="Cost analysis period in days")]
    [ValidateRange(7, 90)]
    [int]$WECostAnalysisDays = 30,
    
    [Parameter(Mandatory=$false, HelpMessage="Generate executive summary report")]
    [switch]$WEExecutiveReport,
    
    [Parameter(Mandatory=$false, HelpMessage="Export detailed CSV reports")]
    [switch]$WEExportCSV,
    
    [Parameter(Mandatory=$false, HelpMessage="Minimum cost threshold for recommendations")]
    [decimal]$WEMinCostThreshold = 50.00,
...
function Get-WERightsizingRecommendations -ErrorAction Stop {
    ...
    try {
        ...
        foreach ($vm in $vms) {
            ...
            $vmRecommendation.Recommendations += "Enable Azure Monitor for detailed utilization metrics"
...
Write-WEOptimizationLog "Wesley Ellis Azure Resource Optimization Analyzer v$WEVersion Starting" "INFO"
Write-WEOptimizationLog "Author: Wesley Ellis | Contact: wesellis.com" "INFO"
Write-WEOptimizationLog "Analysis Type: $WEAnalysisType | Scope: $(if($WEResourceGroupName){"Resource Group: $WEResourceGroupName"}else{"Full Subscription"})" "INFO"
...
try {
    ...
    $WEOptimizationResults = @{
        ...
        AnalysisMetadata = @{
            AnalysisType = $WEAnalysisType
            ExecutionTime = $WEStartTime
            ToolVersion = $WEVersion
            Author = "Wesley Ellis"
            Contact = "wesellis.com"
        }
        Results = @{}
    ...
    switch ($WEAnalysisType) {
        "CostAnalysis" {
            $WEOptimizationResults.Results.CostAnalysis = Get-WEResourceCostAnalysis -SubscriptionId $currentContext.Subscription.Id -ResourceGroupName $WEResourceGroupName -DaysBack $WECostAnalysisDays
        }
        "RightsizingRecommendations" {
            $WEOptimizationResults.Results.RightsizingRecommendations = Get-WERightsizingRecommendations -ResourceGroupName $WEResourceGroupName
        }
        "UnusedResources" {
            $WEOptimizationResults.Results.UnusedResources = Find-WEUnusedResources -ResourceGroupName $WEResourceGroupName
        }
        "FullOptimization" {
            Write-WEOptimizationLog "Performing comprehensive optimization analysis" "ANALYZE"
            $WEOptimizationResults.Results.CostAnalysis = Get-WEResourceCostAnalysis -SubscriptionId $currentContext.Subscription.Id -ResourceGroupName $WEResourceGroupName -DaysBack $WECostAnalysisDays
            $WEOptimizationResults.Results.RightsizingRecommendations = Get-WERightsizingRecommendations -ResourceGroupName $WEResourceGroupName
            $WEOptimizationResults.Results.UnusedResources = Find-WEUnusedResources -ResourceGroupName $WEResourceGroupName
        }
    ...
    if ($WEOptimizationResults.Results.RightsizingRecommendations) {
        $totalPotentialSavings += ($WEOptimizationResults.Results.RightsizingRecommendations | Measure-Object -Property PotentialSavings -Sum).Sum
    }
    if ($WEOptimizationResults.Results.UnusedResources) {
        $totalPotentialSavings += ($WEOptimizationResults.Results.UnusedResources | Measure-Object -Property EstimatedMonthlyCost -Sum).Sum
    }
    ...
    Write-WEOptimizationLog "   Contact: wesellis.com for detailed optimization consulting" "SUCCESS"
    
    # Export results if requested
    if ($WEExportCSV -and $WEOptimizationResults.Results.RightsizingRecommendations) {
        $csvPath = "$env:TEMP\WE-Azure-Optimization-$(Get-Date -Format 'yyyyMMdd-HHmmss').csv"
        $WEOptimizationResults.Results.RightsizingRecommendations | Export-Csv -Path $csvPath -NoTypeInformation
        Write-WEOptimizationLog "Detailed CSV report exported: $csvPath" "SUCCESS"
    }
    
    if ($WEExecutiveReport) {
        $reportPath = "$env:TEMP\WE-Azure-Executive-Report-$(Get-Date -Format 'yyyyMMdd-HHmmss').json"
        $WEOptimizationResults | ConvertTo-Json -Depth 10 | Out-File -FilePath $reportPath -Encoding UTF8
