...
function WE-Trace-Log([string] $msg)
{
    $now = Now-Value
    try
    {
...
function WE-Run-Process([string] $process, [string] $arguments)
{
...
function WE-Download-Gateway([string] $url, [string] $gwPath)
{
    try
    {
...
function WE-Install-Gateway([string] $gwPath)
{
...
function WE-Get-RegistryProperty([string] $keyPath, [string] $property)
{
