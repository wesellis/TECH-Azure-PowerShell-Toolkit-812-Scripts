...
if($Environment -and (Get-AzureRMEnvironment -Name $Environment) -eq $null)
{
   ...
   $Environment = (Get-AzureRMEnvironment -ErrorAction Stop | Select-Object Name, ManagementPortalUrl | Out-GridView -title "Select a valid Azure environment for your subscription" -OutputMode Single).Name
}
...
else # if  resume
{
    ...
    if(-not ($targetResourceGroup = Get-AzureRmResourceGroup -ErrorAction Stop  -ResourceGroupName $NewResourceGroupName))
    {
    ...
    $VHDstorageObjects = (get-content -ErrorAction Stop $VHDstorageObjectsResumePath -ea SilentlyContinue) -Join " `n" | ConvertFrom-Json
}
