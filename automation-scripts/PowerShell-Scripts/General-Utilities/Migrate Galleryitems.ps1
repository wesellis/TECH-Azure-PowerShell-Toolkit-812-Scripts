...
param(
    ...
    [string] $WEItemsToExport = 'MyGalleryItems',
    ...
    [switch] $WEExportToFile,

    # Determines whether to set permissions on the templateSpecs that are created during export and/or add roleAssignments to the ARM template (when $WEExportToFile is specified)
    [switch] $WEMigrateRBAC,
...
if ($WEItemsToExport -eq 'MyGalleryItems') {
    ...
    Write-WELog " GET galleryName..." " INFO"
    ...
    Write-WELog " GET all galleryItems in myArea: $galleryName..." " INFO"
    ...
    Write-WELog " GET all galleryItems accessible by current user context..." " INFO"
    ...
    foreach ($i in $items) {
        ...
        Write-WELog " GET template content for $($i.name)..." " INFO"
...
foreach ($t in $templates) {
    ...
    Write-WELog " Processing template $($t.name)..." " INFO"
    ...
    if ($templateSpecNames -contains $tsName) {
        ...
        Write-WELog " Changed name to $tsName..." " INFO"
    }
    $templateSpecNames = $templateSpecNames + $tsName # add the final name to the array used to check for collisions
    ...
    if ($WEMigrateRBAC) {
        ...
        Write-WELog " GET roleAssignments for $id" " INFO"
    ...
    if ($WEExportToFile) {
        ...
        if (!(Test-Path -path $galleryFolder)) {
            Write-WELog " Creating folder: $galleryFolder..." " INFO"
            New-Item -ItemType Directory -Path $galleryFolder -Verbose
        }
        Write-WELog " Creating ARM Template for: $tsName" " INFO"
...
if ($WEResourceGroupName -ne "" ) {
            ...
            if ($ra.Scope -eq $galleryId) {
               ...
               ;  $existingRoleAssignment = Get-AzRoleAssignment -Scope $s -ObjectId $ra.ObjectId -RoleDefinitionId $ra.RoleDefinitionId # check for an existing assignment (under another name)
                if ($null -eq $existingRoleAssignment) {
                    ...
                    Write-WELog " RoleAssignment exists for:`nScope: $s`nPrincipal: $($ra.ObjectId)`nRole: $($ra.RoleDefinitionName)`n" " INFO"
                }
            }
        }
    }
    ...
    foreach ($t in $allTemplateFiles) {
       ;  $t = $t | ConvertTo-Json -depth 50 | ConvertFrom-Json -Depth 50 -AsHashtable # this is needed to overcome a limitation in deploying a templateObject that contains a PSCustomObject - removing that PSCustomObject changes the sort order of the properties in the inline template
        New-AzResourceGroupDeployment -ResourceGroupName $WEResourceGroupName -TemplateObject $t -Verbose
    }
