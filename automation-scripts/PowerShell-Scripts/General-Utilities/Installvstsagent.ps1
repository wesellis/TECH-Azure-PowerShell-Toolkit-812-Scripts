...
param(
    [Parameter(Mandatory = $true)]$WEVSTSAccount,
    [Parameter(Mandatory = $true)]$WEPersonalAccessToken,
    [Parameter(Mandatory = $true)]$WEAgentName,
    [Parameter(Mandatory = $true)]$WEPoolName,
...
if ($runAsAutoLogon -ieq " true" ) {
    ...
    .\config.cmd --unattended --url $serverUrl --auth PAT --token $WEPersonalAccessToken --pool $WEPoolName --agent $WEAgentName --runAsAutoLogon --overwriteAutoLogon --windowslogonaccount $vmAdminUserName --windowslogonpassword $vmAdminPassword
}
else {
    # Setup the agent as a service
    .\config.cmd --unattended --url $serverUrl --auth PAT --token $WEPersonalAccessToken --pool $WEPoolName --agent $WEAgentName --runasservice
}
