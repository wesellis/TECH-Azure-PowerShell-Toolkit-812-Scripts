...
if($OptionalSourceEnvironment -and (Get-AzureRMEnvironment -Name $OptionalSourceEnvironment) -eq $null)
{
   ...
   $OptionalSourceEnvironment = (Get-AzureRMEnvironment -ErrorAction Stop | Select-Object Name, ManagementPortalUrl | Out-GridView -title "Select a valid Azure environment for your source subscription" -OutputMode Single).Name
}
...
if($sub.count -gt 1)
{
    $SubscriptionId = (Get-AzureRmSubscription -ErrorAction Stop | Select-Object * | Out-GridView -title "Select Target Subscription" -OutputMode Single).Id
...
foreach($vm in $resourceGroupVMs)
{
    ...
    foreach($md in $resourceGroupManagedDisks)
    {
        ...
        [array]$sourceMDstorageObjects = $sourceMDstorageObjects + $PSobjMDstorage
    }
...
if($OptionalTargetEnvironment -and (Get-AzureRMEnvironment -Name $OptionalTargetEnvironment) -eq $null)
{
   ...
   $OptionalTargetEnvironment = (Get-AzureRMEnvironment -ErrorAction Stop | Select-Object Name, ManagementPortalUrl | Out-GridView -title "Select a valid Azure environment for your target subscription" -OutputMode Single).Name
}
...
Create new destination storage accounts
   ...
   ################################>
    ...
    foreach($VHDsrcStorageAccountObj in $VHDsrcStorageAccounts)
    {
        ...
        {
            [array]$sourceStorageObjects = $sourceStorageObjects + $sourceVHDstorageObjects|Where-Object{$_.srcStorageAccount -eq $VHDsrcStorageAccountName}
        }
    }
    ...
    foreach($srcStorageAccountObj in $srcStorageAccounts)
    {
        ...
        foreach($obj in $sourceVHDstorageObjects | Where-Object{$_.srcStorageAccount -eq $srcStorageAccount})
        {
            ...
            [array]$VHDstorageObjects = $VHDstorageObjects + $PSobjVHDstorage
        }
...
Create new network resources.
    ...
    foreach($srcAVset in $resourceGroupAvSets)
    {
        ...
        try
        {
            ...
            $NewAvailabilitySet = New-AzureRmAvailabilitySet -ErrorAction Stop @avParams
    ...
    foreach($srcPIP in $resourceGroupPIPs)
    {
        ...
        try
        {
            ...
            $PIP = New-AzureRmPublicIpAddress -ErrorAction Stop @pipParams
    ...
    throw
        ...
        try
        {
            ...
            $NewLB = New-AzureRmLoadBalancer -ErrorAction Stop @LBparams -ea Stop -wa SilentlyContinue
    ...
    throw
            ...
            try
            {
                ...
                $NewIpConfig = New-AzureRmNetworkInterfaceIpConfig -ErrorAction Stop @ipConfigParams -ea Stop -wa SilentlyContinue
                [array]$NewIpConfigs = $NewIpConfigs + $NewIpConfig
        ...
        try
        {
            ...
            $NewNIC = New-AzureRmNetworkInterface -ErrorAction Stop @NICparams -ea Stop -wa SilentlyContinue
    ...
    foreach($vm in $resourceGroupVMs)
    {
        ...
        [array]$sourceVMSizeObjects = $sourceVMSizeObjects + $PSobjVmSize
    }
...
else # if not resume
{
    ...
    try
    {
        $VHDstorageObjects = (get-content -ErrorAction Stop $VHDstorageObjectsResumePath -ea Stop) -Join " `n" | ConvertFrom-Json
    }
    catch
    {
        $_
...
foreach($srcVM in $resourceGroupVMs)
{
    ...
    foreach($nicID in $srcVM.NetworkProfile.NetworkInterfaces.id)
    {
        ...
        $newNICs = $newNICs + Get-AzureRmNetworkInterface -Name $NICName -ResourceGroupName $ResourceGroupName
    }
    # create VM Config
    if($AvailabilitySet)
    {
