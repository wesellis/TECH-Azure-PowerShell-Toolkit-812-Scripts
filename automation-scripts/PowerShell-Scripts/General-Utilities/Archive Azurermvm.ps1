...
function New-VM -ErrorAction Stop
{ [CmdletBinding()]
param($vmObj)
    $created = $false
...
if($OptionalEnvironment -and (Get-AzureRMEnvironment -Name $OptionalEnvironment) -eq $null)
{
   ...
   $OptionalEnvironment = (Get-AzureRMEnvironment -ErrorAction Stop | Select-Object Name, ManagementPortalUrl | Out-GridView -title "Select a valid Azure environment for your source subscription" -OutputMode Single).Name
}
        ...
        foreach($StorageContainer in (Get-AzureStorageContainer -Context $StorageContext) )
        {
            ...
            foreach($rehydrateBlob in $rehydrateBlobs)
            {
                ...
                try
                {
                    ...
                    $fileContent = get-content -ErrorAction Stop " $env:temp\$TempRehydratefileName" -ea Stop
                    $fileContent | Where-Object{$_ -ne ''} | out-file " $env:temp\$TempRehydratefileName"
                    $rehydrateVM = (get-content -ErrorAction Stop " $env:temp\$TempRehydratefileName" -ea Stop) -Join " `n" | ConvertFrom-Json -ea Stop
                }
                catch
                {
                    ...
                    if($created)
                    {
                        try
                        {
                            ...
                            if($rehydrateDiagBlob)
                            {
                                ...
                                $DiagfileContent = get-content -ErrorAction Stop " $env:temp\$TempDiagRehydratefileName" -ea Stop
                                $DiagfileContent | Where-Object{$_ -ne ''} | out-file " $env:temp\$TempDiagRehydratefileName"
                                $rehydrateVMDiag = (get-content -ErrorAction Stop " $env:temp\$TempDiagRehydratefileName" -ea Stop) -Join " `n" | ConvertFrom-Json
