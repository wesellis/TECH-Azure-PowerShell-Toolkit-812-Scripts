<#
.SYNOPSIS
    We Enhanced Install Shibboleth Idp
...
param(
	...
	{
		$WETempPassword = $WETempPassword + ($sourcedata | GET-RANDOM)
	}
    return $WETempPassword
}
...
$WEJDK_PATH=" -10.0.1"
[System.Environment]::SetEnvironmentVariable(" JAVA_HOME", " c:\Program Files\Java\jdk$WEJDK_PATH", " Machine")
[System.Environment]::SetEnvironmentVariable(" PATH", $WEEnv:Path + " ;c:\Program Files\Java\jdk$WEJDK_PATH\bin", " Machine")
...
$WESSLKEYPASSWORD=GET-Password -length 12 -sourcedata $alphabet
cd " C:\Program Files\Java\jdk$WEJDK_PATH\bin\"
.\keytool.exe -genkey -alias tomcat -keyalg RSA -keystore c:\Temp\server.keystore -keysize 2048 -storepass $WESSLKEYPASSWORD -keypass $WESSLKEYPASSWORD -dname " cn=$WESITENAME, ou=shibbolethOU, o=shibbolethO, c=US"
...
$WEOriginalString='redirectPort=" 8443"'
$WEReplceString='redirectPort=" 8443" address=" 0.0.0.0"'
$filedata=$filedata.Replace($WEOriginalString,$WEReplceString)

$WEOriginalString=" <!-- Define an AJP 1.3 Connector on port 8009 -->"
$WEReplaceWith='<Connector port=" 8443" protocol=" org.apache.coyote.http11.Http11Protocol" SSLEnabled=" true" maxThreads=" 150" scheme=" https" secure=" true"  clientAuth=" false" sslProtocol=" TLS" address=" 0.0.0.0" keystoreFile=" C:\Temp\server.keystore"' + " keystorePass='$WESSLKEYPASSWORD'/>"
$filedata=$filedata.Replace($WEOriginalString,$WEReplaceWith)
...
[IO.File]::AppendAllText(" C:\shibboleth-identity-provider-3.3.2\bin\temp.properties"," idp.sealer.storePassword= $WESSLKEYPASSWORD"+ $newLine)
[IO.File]::AppendAllText(" C:\shibboleth-identity-provider-3.3.2\bin\temp.properties"," idp.sealer.keyPassword= $WESSLKEYPASSWORD" + $newLine)
...
[IO.File]::AppendAllText(" C:\shibboleth-identity-provider-3.3.2\bin\temp.properties"," idp.entityID= https://$WESITENAME/idp/shibboleth" + $newLine)
[IO.File]::AppendAllText(" C:\shibboleth-identity-provider-3.3.2\bin\temp.properties"," idp.scope= $WESITENAME" + $newLine)
...
echo " idp.sealer.password = $WESSLKEYPASSWORD" >C:\shibboleth-identity-provider-3.3.2\credentials.properties
...
$WEOriginalString=" setlocal"
$WEReplceString=" setlocal`r`nset JAVA_HOME=C:\Program Files\Java\jdk-10.0.1"
$filedata=$filedata.Replace($WEOriginalString,$WEReplceString)
[IO.File]::WriteAllText(" C:\shibboleth-identity-provider-3.3.2\bin\install.bat", $filedata.TrimEnd())

cmd.exe /C " C:\shibboleth-identity-provider-3.3.2\bin\install.bat -Didp.src.dir=C:\shibboleth-identity-provider-3.3.2 -Didp.target.dir=C:\opt\shibboleth-idp\ -Didp.merge.properties=C:\shibboleth-identity-provider-3.3.2\bin\temp.properties -Didp.sealer.password=$WESSLKEYPASSWORD -Didp.keystore.password=$WESSLKEYPASSWORD -Didp.conf.filemode=644 -Didp.host.name=$WESITENAME -Didp.scope=$WESITENAME"
...
$WEOriginalString=" https://$WESITENAME/idp/profile/Shibboleth/SSO"
$WEReplceString=" https://$WESITENAME"+" :8443/idp/profile/Shibboleth/SSO"
$content=$content.Replace($WEOriginalString,$WEReplceString)

$WEOriginalString=" https://$WESITENAME/idp/profile/SAML2/POST/SSO"
$WEReplceString=" https://$WESITENAME"+" :8443/idp/profile/SAML2/POST/SSO"
$content=$content.Replace($WEOriginalString,$WEReplceString)

$WEOriginalString=" https://$WESITENAME/idp/profile/SAML2/POST-SimpleSign/SSO"
$WEReplceString=" https://$WESITENAME"+" :8443/idp/profile/SAML2/POST-SimpleSign/SSO"
$content=$content.Replace($WEOriginalString,$WEReplceString)

$WEOriginalString=" https://$WESITENAME/idp/profile/SAML2/Redirect/SSO"
$WEReplceString=" https://$WESITENAME"+" :8443/idp/profile/SAML2/Redirect/SSO"
$content=$content.Replace($WEOriginalString,$WEReplceString)
...
$WEOriginalString=" '::1/128'"
$WEReplceString=" '::1/128', '0.0.0.0/0'"
$content=$content.Replace($WEOriginalString,$WEReplceString)
