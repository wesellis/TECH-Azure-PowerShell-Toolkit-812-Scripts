...
function WE-ValidateKeyVaultAndCreate([string] $keyVaultName, [string] $resourceGroup, [string] $WEKeyVaultLocation) 
{
   $WEGetKeyVault=Get-AzureRmKeyVault -VaultName $keyVaultName -ResourceGroupName $resourceGroup -ErrorAction SilentlyContinue
   if (!$WEGetKeyVault)
   {
 ...
 function WE-CreateSelfSignedCertificate([string] $keyVaultName, [string] $certificateName, [string] $selfSignedCertPlainPassword,[string] $certPath, [string] $certPathCer, [string] $noOfMonthsUntilExpired ) 
{
   ...
   $WERemoveAzureKeyVaultCertificateStatus = Remove-AzureKeyVaultCertificate -VaultName $keyVaultName -Name $certificateName -PassThru -Force -ErrorAction SilentlyContinue -Confirm:$false
 }
 ...
 function WE-CreateServicePrincipal([System.Security.Cryptography.X509Certificates.X509Certificate2] $WEPfxCert, [string] $applicationDisplayName) {  
   ...
   $WEKeyCredential.CertValue  = $keyValue

   # Use Key credentials and create AAD Application
   $WEApplication = New-AzureRmADApplication -DisplayName $WEApplicationDisplayName -HomePage (" http://" + $applicationDisplayName) -IdentifierUris (" http://" + $WEKeyId) -KeyCredentials $WEKeyCredential

   $WEServicePrincipal = New-AzureRMADServicePrincipal -ApplicationId $WEApplication.ApplicationId 
   $WEGetServicePrincipal = Get-AzureRmADServicePrincipal -ObjectId $WEServicePrincipal.Id
  ...
  ;  $WERetries = 0;
   While ($null -eq $WENewRole -and $WERetries -le 6)
   {
      ...
      Start-Sleep -s 10
      $WENewRole = Get-AzureRMRoleAssignment -ServicePrincipalName $WEApplication.ApplicationId -ErrorAction SilentlyContinue
      $WERetries++;
   }

   return $WEApplication.ApplicationId.ToString();
 }

 function WE-CreateAutomationCertificateAsset ([string] $resourceGroup, [string] $automationAccountName, [string] $certifcateAssetName,[string] $certPath, [string] $certPlainPassword, [Boolean] $WEExportable) {
   $WECertPassword = ConvertTo-SecureString $certPlainPassword -AsPlainText -Force   
   Remove-AzureRmAutomationCertificate -ResourceGroupName $resourceGroup -automationAccountName $automationAccountName -Name $certifcateAssetName -ErrorAction SilentlyContinue
   New-AzureRmAutomationCertificate -ResourceGroupName $resourceGroup -automationAccountName $automationAccountName -Path $certPath -Name $certifcateAssetName -Password $WECertPassword -Exportable:$WEExportable  | write-verbose
 }

 function WE-CreateAutomationConnectionAsset ([string] $resourceGroup, [string] $automationAccountName, [string] $connectionAssetName, [string] $connectionTypeName, [System.Collections.Hashtable] $connectionFieldValues ) {
   Remove-AzureRmAutomationConnection -ResourceGroupName $resourceGroup -automationAccountName $automationAccountName -Name $connectionAssetName -Force -ErrorAction SilentlyContinue
   New-AzureRmAutomationConnection -ResourceGroupName $WEResourceGroup -automationAccountName $automationAccountName -Name $connectionAssetName -ConnectionTypeName $connectionTypeName -ConnectionFieldValues $connectionFieldValues 
 }
...
try
{
    ...
    if ($null -eq $servicePrincipalConnection)
    {
        ...
        $WEAzureLoginUserName = $myCredential.UserName
        $securePassword = $myCredential.Password
        $WEAzureLoginPassword = $myCredential.GetNetworkCredential().Password
        ...
        try
         {
            ...
            $WEKeyVaultLocation = $WERG[0].Location
 
            # Create Run As Account using Service Principal
            $WECertifcateAssetName = " AzureRunAsCertificate"
            $WEConnectionAssetName = " AzureRunAsConnection"
            $WEConnectionTypeName = " AzureServicePrincipal"
            ...
            $WEPfxCert = New-Object -TypeName System.Security.Cryptography.X509Certificates.X509Certificate2 -ArgumentList @($WEPfxCertPathForRunAsAccount, $WEPfxCertPlainPasswordForRunAsAccount)
            $WEApplicationId=CreateServicePrincipal $WEPfxCert $WEApplicationDisplayName

            Write-Output " Creating Certificate in the Asset..."
            # Create the automation certificate asset
            CreateAutomationCertificateAsset $aroResourceGroupName $automationAccountName $WECertifcateAssetName $WEPfxCertPathForRunAsAccount $WEPfxCertPlainPasswordForRunAsAccount $true
           ...
           ;  $WEConnectionFieldValues = @{" ApplicationId" = $WEApplicationId; " TenantId" = $WETenantID.TenantId; " CertificateThumbprint" = $WEThumbprint; " SubscriptionId" = $WESubscriptionId} 
            ...
            CreateAutomationConnectionAsset $aroResourceGroupName $automationAccountName $WEConnectionAssetName $WEConnectionTypeName $WEConnectionFieldValues
    ...
    else
    {
        ...
        try
        {
            ...
            $params = @{
                ...
                Message = $_.Exception throw $_.Exception exit } } } #++++++++++++++++++++++++STEP 1 execution ends++++++++++++++++++++++++++
            }
    ...
    try
    {
        ...
        [String] $WEWebhookUriVariableName =" Internal_AutoSnooze_WebhookUri"
        ...
        if($null -eq $checkWebhook)
        {
            ...
            $WEExpiryTime = (Get-Date).AddDays(730)

            Write-Output " Creating the Webhook ($($webhookNameforStopVM)) for the Runbook ($($runbookNameforStopVM))..."
            $WEWebhookdata = New-AzureRmAutomationWebhook -Name $webhookNameforStopVM -automationAccountName $automationAccountName -ResourceGroupName $aroResourceGroupName -RunbookName $runbookNameforStopVM -IsEnabled $true -ExpiryTime $WEExpiryTime -Force
            Write-Output " Successfully created the Webhook ($($webhookNameforStopVM)) for the Runbook ($($runbookNameforStopVM))..."
    
            $WEServiceUri = $WEWebhookdata.WebhookURI

            Write-Output " Webhook Uri [$($WEServiceUri)]"

            Write-Output " Creating the Assest Variable ($($WEWebhookUriVariableName)) in the Automation Account ($($automationAccountName)) to store the Webhook URI..."
            New-AzureRmAutomationVariable -automationAccountName $automationAccountName -Name $WEWebhookUriVariableName -Encrypted $WEFalse -Value $WEServiceUri -ResourceGroupName $aroResourceGroupName
            Write-Output " Successfully created the Assest Variable ($($WEWebhookUriVariableName)) in the Automation Account ($($automationAccountName)) and Webhook URI value updated..."
    ...
    try
    {
        ...
        if($null -eq $checkMegaSchedule)
        {
            ...
            $WEStartTime = (Get-Date).AddMinutes(10)
            $WEEndTime = $WEStartTime.AddYears(3)
            ...
            New-AzureRmAutomationSchedule -automationAccountName $automationAccountName -Name $scheduleNameforCreateAlert -ResourceGroupName $aroResourceGroupName -StartTime $WEStartTime -ExpiryTime $WEEndTime -HourInterval $WEHours
    ...
    try
    {
       ...
       ;  $WEStopVmUTCTime = (Get-Date -ErrorAction Stop " 01:00:00" ).AddDays(1).ToUniversalTime()
        ...
        if($null -eq $checkSchSnoozeStart)
        {
            ...
            New-AzureRmAutomationSchedule -automationAccountName $automationAccountName -Name $scheduleStart -ResourceGroupName $aroResourceGroupName -StartTime $WEStartVmUTCTime -ExpiryTime $WEStartVmUTCTime.AddYears(1) -DayInterval 1
    ...
    try
    {
        ...
        if($null -eq $checkScheduleAU)
        {
            ...
            New-AzureRmAutomationSchedule -automationAccountName $automationAccountName -Name $scheduleNameforAutoupdate -ResourceGroupName $aroResourceGroupName -StartTime $WEStartUTCTime -WeekInterval 2
    ...
    try
    {
       ...
       ;  $WEStopVmUTCTime = (Get-Date -ErrorAction Stop " 01:00:00" ).AddDays(1).ToUniversalTime()
        ...
        if($null -eq $checkSeqSnoozeStart)
        {
            ...
            #---Create the schedule at the Automation Account level--- 
            Write-Output " Creating the Schedule in Automation Account ($($automationAccountName))..."
            New-AzureRmAutomationSchedule -automationAccountName $automationAccountName -Name $sequenceStart -ResourceGroupName $aroResourceGroupName -StartTime $WEStartVmUTCTime -DaysOfWeek Monday -WeekInterval 1
        ...
        if($null -eq $checkSeqSnoozeStop)
        {
            ...
            #---Create the schedule at the Automation Account level--- 
            Write-Output " Creating the Schedule in Automation Account ($($automationAccountName))..."
            New-AzureRmAutomationSchedule -automationAccountName $automationAccountName -Name $sequenceStop -ResourceGroupName $aroResourceGroupName -StartTime $WEStopVmUTCTime -DaysOfWeek Friday -WeekInterval 1
    ...
    try{
        ...
        $WEStatus = (Get-AzureRmDiagnosticSetting -ResourceID $automationAccountId | Select-Object -ExpandProperty Logs | Where-Object {$_.Enabled -eq $false}).Count
        if ($WEStatus -gt 0) 
        {
