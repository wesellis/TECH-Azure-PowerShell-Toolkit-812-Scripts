...
function WE-Unzip-Archive($archive, $destination){
...
function WE-SetEnv-JavaHome($jdkInstallLocation)
{
...
function WE-Install-ElasticSearch ($driveLetter, $elasticSearchZip, $subFolder = $elasticSearchBaseFolder)
{
...
function WE-ElasticSearch-InstallService($scriptPath)
{
    ...
    {	
        ...
        if ($WELASTEXITCODE) {
            throw " Command '$scriptPath': exit code: $WELASTEXITCODE"
        }
    }
}
...
function WE-Jmeter-Download($drive)
{
	try{
...
function WE-Jmeter-Unzip($source, $drive)
{
    # Unzip now
    $shell = new-object -com shell.application
...
function WE-Jmeter-Run($unzipLoc)
{
