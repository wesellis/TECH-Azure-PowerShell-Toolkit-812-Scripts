...
$certWThumb = $certPath + '\' + $certObj.Thumbprint
  ...
  #          SetScript =  {
		...
		#	}
  #          TestScript = " try { (Get-Item -ErrorAction Stop Cert:\LocalMachine\WebHosting\C534DFBFE8DB597F22320682F7BBFBA2611DC45A -ErrorAction Stop).HasPrivateKey} catch { `$False }"
  #          GetScript = {
