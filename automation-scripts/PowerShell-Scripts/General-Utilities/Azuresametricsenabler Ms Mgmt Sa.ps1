<#
.SYNOPSIS
    We Enhanced Azuresametricsenabler Ms Mgmt Sa
...
If($jsonvhdpool){$WEOMSRES=Post-OMSData -customerId $customerId -sharedKey $sharedKey -body ([System.Text.Encoding]::UTF8.GetBytes($jsonvhdpool)) -logType $logname}
if($WEOMSRES -ge 200 -and $omsres -lt 300)
{
...
Foreach($sa in $salist|Where{$_.properties.accountType -notmatch 'premium' -and $_.sku.tier -ne 'Premium'})
{
	Foreach ($svc in $services)
	{
		...
		[xml]$WEXresponse=invoke-StorageREST -sharedKey $sa.Key -method GET -resource $sa.name -uri $uriprop  -svc $svc
		
		
		IF ($WEXresponse.StorageServiceProperties.MinuteMetrics.Enabled -ne 'true')
		{
