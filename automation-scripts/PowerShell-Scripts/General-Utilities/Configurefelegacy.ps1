...
param(
    ...
    Node localhost
    {
        ...
        if ($WESharePointVersion -eq " 2013") {
            ...
            SetScript =
            {
                ...
                do {
                    try
                    {
                        ...
                        $WEResponse = Invoke-WebRequest -Uri $uri -UseDefaultCredentials -TimeoutSec 10 -ErrorAction Stop -UseBasicParsing
                        # When it will be actually ready, site will respond 401/302/200, and $WEResponse.StatusCode will be 200
                        $currentStatusCode = $WEResponse.StatusCode
                    }
        ...
        Script CreateShortcuts
        {
            SetScript =
            {
                ...
                $WEShortcut.Arguments = " -NoExit -Command "" & 'C:\Program Files\Common Files\Microsoft Shared\Web Server Extensions\$directoryVersion\CONFIG\POWERSHELL\Registration\SharePoint.ps1'"""
                $WEShortcut.Save()
            }
            GetScript            = { return @{ " Result" = " false" } } # This block must return a hashtable. The hashtable must only contain one key Result and the value must be of type String.
            TestScript           = { return $false } # If it returns $false, the SetScript block will run. If it returns $true, the SetScript block will not run.
        ...
        #             $WEShortcut.TargetPath = " C:\Packages\Plugins\Microsoft.Powershell.DSC\{0}\DSCWork\ConfigureFELegacy.0" -f $folderWithMaxVersionNumber
        #             $WEShortcut.Save()
        #         }
        #         GetScript = { }
