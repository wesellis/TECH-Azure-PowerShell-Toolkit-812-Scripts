...
Function ProcessRunner(
  ...
  #>
    ...
    if ($WEExitCode -ne 0) {
        ...
        Write-Output " Exit code: $WEExitCode"
        Write-Output " **ERROR**"
...
param(
    ...
    return ($WERepoUrl -Match '^https://[a-zA-Z][\w\-_]*\.visualstudio\.com/.*' -or $WERepoUrl -Match '^https://dev\.azure\.com/.*')
}
...
param(
    ...
    if ($repoSparseCheckout -ne $baseRepoSparseCheckout) {
        Write-WELog " Base image sparse checkout configuration: $baseRepoSparseCheckout" " INFO"
        Write-WELog " Image sparse checkout configuration: $repoSparseCheckout" " INFO"
    ...
    elseif ($commitId -ne 'latest') {
        Write-WELog " Fetching commit $commitId" " INFO"
...
param(
    ...
    if (!(Test-Path -Path $targetDirectory -PathType Container)) {
        ...
        if ($? -eq $false) {
            if ($cloneIfNotExists -eq $true) {
                ...
                Write-WELog " repository not found at '$targetDirectory'." " INFO"
    ...
    if ($shouldCloneRepo -eq $true) {
        ...
        ConfigureGitRepoBeforeClone -gitExeLocation $WEGitExeLocation
        switch ($sourceControl) {
            git {
                CloneGitRepo -gitExeLocation $WEGitExeLocation -gitRepoLocation $repoUrl -gitLocalRepoLocation $targetDirectory -gitBranchName $gitBranchName -optionalGitCloneArgs $optionalCloneArgs -formattedSparseCheckoutFolders $formattedSparseCheckoutFolders -msiClientId $msiClientId
            }
            gvfs {
                GvfsCloneGitRepo -gitExeLocation $WEGitExeLocation -gvfsExeLocation $WEGvfsExeLocation -gvfsRepoLocation $repoUrl -gvfsLocalRepoLocation $targetDirectory -gitBranchName $gitBranchName -msiClientId $msiClientId
    ...
    try {
        ...
        switch ($repository_SourceControl) {
            { ($_ -eq [SourceControl]::git) -or ($_ -eq [SourceControl]::gvfs) } {
                ...
                ProcessRunner -command $WEGitExeLocation -arguments " version"
                if ($? -ne $true) {
            ...
            { $_ -eq [SourceControl]::gvfs } {
                ...
                ProcessRunner -command $WEGvfsExeLocation -arguments " version"
        ...
        if (!([string]::IsNullOrWhiteSpace($branchName))) {
            Write-WELog " Use explicitly provided branch '$branchName' rather than commitId" " INFO"
            $commitId = 'latest'
        }
