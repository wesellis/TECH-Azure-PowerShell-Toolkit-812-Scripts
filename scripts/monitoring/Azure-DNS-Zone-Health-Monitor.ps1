...
Write-Host "  [OK] SOA Record: $(if ($HasSOA) { 'Present' } else { 'Missing' })"
Write-Host "  [OK] NS Records: $(if ($HasNS) { 'Present' } else { 'Missing' })"
Write-Host "  [OK] Root A Record: $(if ($HasA) { 'Present' } else { 'Missing (Optional)' })"
Write-Host "  [OK] WWW Record: $(if ($HasWWW) { 'Present' } else { 'Missing (Recommended)' })"
# Check TTL values
$LowTTLRecords = $RecordSets | Where-Object { $_.Ttl -lt 300 -and $_.RecordType -ne "SOA" }
if ($LowTTLRecords.Count -gt 0) {
    Write-Host "  [WARN] Low TTL Warning: $($LowTTLRecords.Count) records have TTL < 5 minutes"
...
if ($HighTTLRecords.Count -gt 0) {
    Write-Host "  [WARN] High TTL Warning: $($HighTTLRecords.Count) records have TTL > 24 hours"
}
# DNS resolution test
...
try {
    ...
    Write-Host "  [WARN] DNS resolution test failed: $($_.Exception.Message)"
}
