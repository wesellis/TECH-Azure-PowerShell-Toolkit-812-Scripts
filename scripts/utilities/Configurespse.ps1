...
param(
    ...
    # Init
    [String] $InterfaceAlias = (Get-NetAdapter -ErrorAction Stop | Where-Object InterfaceDescription -Like "Microsoft Hyper-V Network Adapter*" | Select-Object -First 1).Name
    ...
    [System.Management.Automation.PSCredential] $SPSetupCredsQualified = New-Object -ErrorAction Stop System.Management.Automation.PSCredential (" $DomainNetbiosName\$($SPSetupCreds.UserName)" , $SPSetupCreds.Password)
    [System.Management.Automation.PSCredential] $SPFarmCredsQualified = New-Object -ErrorAction Stop System.Management.Automation.PSCredential (" $DomainNetbiosName\$($SPFarmCreds.UserName)" , $SPFarmCreds.Password)
    [System.Management.Automation.PSCredential] $SPSvcCredsQualified = New-Object -ErrorAction Stop System.Management.Automation.PSCredential (" $DomainNetbiosName\$($SPSvcCreds.UserName)" , $SPSvcCreds.Password)
    [System.Management.Automation.PSCredential] $SPAppPoolCredsQualified = New-Object -ErrorAction Stop System.Management.Automation.PSCredential (" $DomainNetbiosName\$($SPAppPoolCreds.UserName)" , $SPAppPoolCreds.Password)
    [System.Management.Automation.PSCredential] $SPADDirSyncCredsQualified = New-Object -ErrorAction Stop System.Management.Automation.PSCredential (" $DomainNetbiosName\$($SPADDirSyncCreds.UserName)" , $SPADDirSyncCreds.Password)
    # Setup settings
...
$dnsRecordFound = $true
        ...
        Registry ShowFileExtensions {
            Key = "HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\" ; ValueName = "HideFileExt" ; ValueType = "DWORD" ; ValueData = " 0" ; Force = $true;
        ...
        ADUser CreateSParmAccount {
            ...
            UserPrincipalName    = " $($SPFarmCreds.UserName)@WEDomainFQDN"
        ...
        ADUser CreateSPSvcAccount {
            ...
            UserPrincipalName    = " $($SPSvcCreds.UserName)@WEDomainFQDN"
            Password             = $SPSvcCreds
            PasswordNeverExpires = $true
            Ensure               = "Present"
            PsDscRunAsCredential = $DomainAdminCredsQualified
        ...
        ADUser CreateSPAppPoolAccount {
            ...
            UserPrincipalName     = " $($SPAppPoolCreds.UserName)@WEDomainFQDN"
        ...
        ADUser CreateSPSuperUserAccount {
            ...
            UserPrincipalName    = " $($SPSuperUserCreds.UserName)@WEDomainFQDN"
        ...
        ADUser CreateSPSuperReaderAccount {
            ...
            UserPrincipalName    = " $($SPSuperReaderCreds.UserName)@WEDomainFQDN"
        ...
        ADUser CreateSPADDirSyncAccount {
            ...
            UserPrincipalName    = " $($SPADDirSyncCreds.UserName)@WEDomainFQDN"
...
$sqlConnection = New-Object -ErrorAction Stop System.Data.SqlClient.SqlConnection "Data Source=$server;Initial Catalog=$db;Integrated Security=True;Enlist=False;Connect Timeout=3"
        ...
        # ExtendMainWebApp might fail with error: "The web.config could not be saved on this IIS Web Site: C:\\inetpub\\wwwroot\\wss\\VirtualDirectories\\80\\web.config.\r
The process cannot access the file 'C:\\inetpub\\wwwroot\\wss\\VirtualDirectories\\80\\web.config' because it is being used by another process."
        ...
        Script GenerateMainWebAppCertificate {
            SetScript            =
            {
                ...
                # $password = Read-Host -Prompt "Enter secure value" -AsSecureString " <superpasse>"
                ...
                $params = @{
                    ...
                    Port = "443"
                    eq = $cert) { return $false   # Run SetScript } else { return $true    # Certificate is already created } } DependsOn            = " [Script]UpdateGPOToTrustRootCACert" , "[SPWebAppAuthentication]ConfigureMainWebAppAuthentication" PsDscRunAsCredential = $DomainAdminCredsQualified }
        ...
        SPSite CreateRootSite {
            ...
            SecondaryOwnerAlias  = " i:0$TrustedIdChar.t|$DomainFQDN|$($DomainAdminCreds.UserName)@WEDomainFQDN"
            Name                 = " root site"
...
$userProfile = $upm.CreateUserProfile($account.AccountName, $account.PreferredName);
        ...
        Script CreateShortcuts {
            SetScript            =
            {
                ...
                $Shortcut.Arguments = " -NoExit -Command "" & '${env:ProgramFiles}\WindowsPowerShell\Modules\SharePointServer\SharePoint.ps1'"
                $Shortcut.Save()
            }
            GetScript            = { return @{ "Result" = " false" } } # This block must return a hashtable. The hashtable must only contain one key Result and the value must be of type String.
            TestScript           = { return $false } # If it returns $false, the SetScript block will run. If it returns $true, the SetScript block will not run.
...
[CmdletBinding()]
function Get-LatestGitHubRelease -ErrorAction Stop {
    [OutputType([string])]
    [CmdletBinding()]
param(
        [string] $Repo,
