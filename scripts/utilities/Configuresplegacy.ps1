...
$dnsRecordFound = $true
        ...
        Registry ShowFileExtensions { Key = "HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\" ; ValueName = "HideFileExt" ; ValueType = "DWORD" ; ValueData = " 0" ; Force = $true; Ensure = "Present" ; PsDscRunAsCredential = $DomainAdminCredsQualified }
            ...
            SetScript =
            {
                ...
                $jobBlock = {
                    ...
                    foreach ($AdditionalUser in $AdditionalUsers) {
                        $accounts = $accounts + $accountPattern_WinClaims -f $AdditionalUser.SamAccountName
                        $accounts = $accounts + $accountPattern_Trusted -f $AdditionalUser.SamAccountName
                    }
...
$profile = $upm.CreateUserProfile($accountName);
        ...
        Script CreateShortcuts
        {
            SetScript =
            {
                ...
                $Shortcut.Arguments = " -NoExit -Command "" & '${env:ProgramFiles}\Common Files\Microsoft Shared\Web Server Extensions\$directoryVersion\CONFIG\POWERSHELL\Registration\SharePoint.ps1'"""
                $Shortcut.Save()
            }
            GetScript            = { return @{ "Result" = " false" } } # This block must return a hashtable. The hashtable must only contain one key Result and the value must be of type String.
            TestScript           = { return $false } # If it returns $false, the SetScript block will run. If it returns $true, the SetScript block will not run.
...
function Get-SPDSCInstalledProductVersion -ErrorAction Stop
{
