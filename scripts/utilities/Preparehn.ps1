...
param(
    [Parameter(Mandatory, ParameterSetName='NodePrepare')]
    [String] $DomainFQDN,
    [Parameter(Mandatory, ParameterSetName='NodePrepare')]
    ...
    [Parameter(Mandatory, ParameterSetName='NodeState')]
...
else
{
    ...
    if(-not (Test-Path -Path $HPCHNDeployRoot))
    {
        ...
        $acl = Get-Acl -ErrorAction Stop $HPCHNDeployRoot
        ...
        try
        {
            $rule = New-Object -ErrorAction Stop System.Security.AccessControl.FileSystemAccessRule(" $domainNetBios\$AdminUserName" ,"FullControl" , "ContainerInherit, ObjectInherit" , "None" , "Allow" )
            $acl.AddAccessRule($rule)
        }
        catch
        {
    ...
    else
    {
        ...
        $params = @{
            ...
            ArgumentList = "@(" $domainNetBios\$AdminUserName" , (ConvertTo-SecureString"
...
#>
                 ...
                 if($null -eq $restWebCert)
                 {
                     ...
                     $thumbprint = . $hpcBinPath
ew-HpcCert.ps1 -MachineName $PublicDnsName -SelfSigned
    ...
    else
    {
        ...
        if((Test-Path $postScriptFlagFile) -and ($PostConfigScript -eq (Get-Content -ErrorAction Stop $postScriptFlagFile | select -First 1)))
        {
