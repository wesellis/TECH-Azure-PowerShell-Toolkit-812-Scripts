...
param(
    ...
    [string] [Parameter(Mandatory)] $SessionHostsOUDistinguishedName
)
$SecureStringPwd = $ServicePrincipalSecret | Read-Host -Prompt "Enter secure value" -AsSecureString
...
$SplitObject = $UserAccess.Split(" ;" )
        $Access = New-Object -ErrorAction Stop Security.AccessControl.FileSystemAccessRule ($SplitObject[0], $SplitObject[1], $SplitObject[2],'None', 'Allow')
