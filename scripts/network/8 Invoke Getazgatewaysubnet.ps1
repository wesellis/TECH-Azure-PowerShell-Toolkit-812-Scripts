...
<#`n.SYNOPSIS
    Invoke Getazgatewaysubnet

.DESCRIPTION
    Invoke Getazgatewaysubnet operation
...
param(
    ...
    [string]$GetAzVirtualNetworkSubnetConfigsplat
    )
    begin {
    }
    process {
        try {
    [string]$GatewaySubnetConfig = Get-AzVirtualNetworkSubnetConfig -ErrorAction Stop @GetAzVirtualNetworkSubnetConfigsplat
        }
        catch {
