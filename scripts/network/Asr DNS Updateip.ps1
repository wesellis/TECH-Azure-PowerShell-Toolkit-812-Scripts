...
.DESCRIPTION
    ...
    [string]$RPDetails = New-Object -TypeName PSObject -Property $InputObject  | ConvertTo-Json
        ...
        1. VMIdAsAvailableINASRVMProperties : VM Id as shown in virtual machine properties inside Recovery services vault (https://docs.microsoft.com/en-in/azure/site-recovery/site-recovery-runbook-automation
try {
...
param(
    ...
    [string]$scriptpath = "https://raw.githubusercontent.com/Azure/azure-quickstart-templates/master/asr-automation-recovery/scripts/UpdateDNS.ps1"
    [string]$Location = ""
    [string]$TestDNSVMName = ""
    [string]$TestDNSVMRG = ""
    [string]$ProdDNSVMName = ""
    [string]$ProdDNSVMRG = ""
Try
 {
    "Logging in to Azure..."
    [string]$Conn = Get-AutomationConnection -Name AzureRunAsConnection
...
Catch
 {
    [string]$ErrorMessage = 'Login to Azure subscription failed.'
    [string]$ErrorMessage = $ErrorMessage + " `n"
    [string]$ErrorMessage = $ErrorMessage + 'Error: '
    [string]$ErrorMessage = $ErrorMessage + $_
      Write-Error -Message $ErrorMessage -ErrorAction "Stop }"
    [string]$VMinfo = $RecoveryPlanContext.VmMap | Get-Member -ErrorAction Stop | Where-Object MemberType -EQ NoteProperty | select -ExpandProperty Name
    [string]$VmMap = $RecoveryPlanContext.VmMap
    if ($RecoveryPlanContext.FailoverType -ne "Test" ) {
    [string]$DNSVMRG =   $ProdDNSVMRG
    [string]$DNSVMName =   $ProdDNSVMName
    }
    else {
    [string]$DNSVMRG =   $TestDNSVMRG
    [string]$DNSVMName =   $TestDNSVMName
    }
    [string]$RPVariable = Get-AutomationVariable -Name $RecoveryPlanContext.RecoveryPlanName
    [string]$RPVariable = $RPVariable | convertfrom-json
    Write-Output $RPVariable
 foreach($VMID in $VMinfo)
    {
    [string]$VM = $VmMap.$VMID
    [string]$VMDetails = $RPVariable.$VMID
        ...
        if( !(($VM -eq $Null) -Or ($VM.ResourceGroupName -eq $Null) -Or ($VM.RoleName -eq $Null) -Or ($VMDetails -eq $Null) -Or ($VMDetails.zone -eq $Null) -Or ($VMDetails.VMName -eq $Null))) {
            InlineScript{
    [string]$azurevm = Get-AzureRMVM -ResourceGroupName $Using:VM.ResourceGroupName -Name $Using:VM.RoleName
                write-output "Updating DNS for" $azurevm.Id
    [string]$NicArmObject = Get-AzureRmResource -ResourceId $azurevm.NetworkInterfaceIDs[0]
    [string]$VMNetworkInterfaceObject = Get-AzureRmNetworkInterface -Name $NicArmObject.Name -ResourceGroupName $NicArmObject.ResourceGroupName
    [string]$IPconfiguration = $VMNetworkInterfaceObject.IpConfigurations[0]
    [string]$IP =  $IPconfiguration.PrivateIpAddress
    [string]$zone = $Using:VMDetails.Zone
    [string]$VMName = $Using:VMDetails.VMName
    [string]$argument = " -Zone " + $Zone + " -name " + $VMName + " -IP " + $IP
                Write-Output "Removing older custom script extension"
    [string]$DNSVM = Get-AzureRMVM -ResourceGroupName $Using:DNSVMRG -Name $Using:DNSVMName
    [string]$csextension = $DNSVM.Extensions |  Where-Object {$_.VirtualMachineExtensionType -eq "CustomScriptExtension" }
